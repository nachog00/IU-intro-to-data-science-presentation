...
class SemiSupervisedGraph():
    ...
    def get_random_points(self):
        ...
        for i in range(self.k):
            ...
            group = VGroup(*[Dot(centroid + self.axes.c2p(*np.random.normal(0,0.1,2)) , radius=0.05 , fill_opacity=0.8 , color=COLORS[i]) for j in range(self.n)])
            points.append(group)
    ...
    def get_middle_line(self):
        ...
        return DashedLine(self.upper_point, self.lower_point , color=WHITE , stroke_width=1.5)
    
    def get_flying_points(self):
        # create a cluster of points that come from the upper middle line into either of the clusters
        self.flying_points = [Dot(self.upper_point + self.axes.c2p(*np.random.normal(0,0.05,2)) , radius=0.05 , fill_opacity=0.8 , color=WHITE) for j in range(self.num_flying_points)]
    ...
    def assign_flying_points(self):
        ...
        for point in self.flying_points:
            point.target = self.centroids[np.random.randint(0,self.k)].get_center() + self.axes.c2p(*np.random.normal(0,0.03,2))
            
            
    def move_flying_points(self):
        self.assign_flying_points()
        # move the flying points to their target
        for point in self.flying_points:
            self.scene.play(MoveAlongPath(point , ArcBetweenPoints(point.get_center() , point.target)))
        
            
    def render_animated(self):
        # self.scene.play(Write(self.axes))
        self.scene.play(FadeIn(self.middle_line))
        self.scene.play(Write(self.centroids) , Write(self.points) , Write(self.flying_points_cluster))
        self.move_flying_points()
        
    def render_static(self):
        self.scene.add(self.middle_line, self.centroids , self.points , self.flying_points_cluster )
    
    def get_graph(self):
        graph = VGroup( self.centroids , self.points , self.flying_points_cluster , self.middle_line )
